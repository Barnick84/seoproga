...
async def create_payment(req: CreatePaymentRequest, current_user: TokenData = Depends(get_current_user)):
    min_amount = 500
    if req.amount < min_amount:
        raise HTTPException(status_code=400, detail=f"Минимальная сумма пополнения {min_amount} руб.")
        
    shop_id = os.getenv("CARDLINK_SHOP_ID")
    token = os.getenv("CARDLINK_TOKEN")
    
    if not shop_id or not token:
        raise HTTPException(status_code=500, detail="Платёжный шлюз не настроен (CARDLINK_SHOP_ID, CARDLINK_TOKEN)")
...
async def payment_successful(
    ...
    try:
        ...
        logger.info(f"Cardlink webhook processed params: InvId={InvId}, OutSum={OutSum}, Status={Status}")
        ...
        try:
            with get_db_cursor(commit=False, dictionary=True) as (conn, cur):
                cur.execute("SELECT id, user_id, amount, currency, order_id, status, created_at FROM payment_history WHERE order_id = %s AND status = %s FOR UPDATE", (order_id, "pending"))
                payments = cur.fetchall()
                ...
                cur.execute("UPDATE users SET balance = balance + %s WHERE id = %s", (amount, payment["user_id"]))
                cur.execute("UPDATE payment_history SET status = %s WHERE id = %s", ("success", payment["id"]))
                cur.execute(
                    "INSERT INTO billing_history (user_id, amount, description, type) VALUES (%s, %s, %s, %s)",
                    (payment["user_id"], amount, f"Пополнение баланса (Заказ {order_id})", "deposit")
                )
                conn.commit()

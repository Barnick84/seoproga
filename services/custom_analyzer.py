...
class CustomAnalyzer:
    ...
    def extract_segments(self, html: str) -> Dict[str, str]:
        ...
        return {"text": self.clean_string(text), "links": self.clean_string(" ".join(links))}
    ...
    def calculate_complexity_metrics(self, lemmas: List[str], text: str) -> Dict[str, Any]:
        ...
        # 1. Academic Stuffing (Академическая тошнота) - frequency of top word
        counts = Counter(lemmas)
        top_word_count = counts.most_common(1)[0][1] if counts else 0
        academic_stuffing = round((top_word_count / len(lemmas)) * 100, 2)
        ...
        return {"academic_stuffing": academic_stuffing, "wateriness": wateriness, "zipf": zipf_score}
    ...
    def get_intent(self, lemmas: List[str], raw_words: List[str] | None = None) -> str:
        ...
        combined_tokens = lemmas + (raw_words if raw_words else [])
    ...
    def analyze_content(self, html: str, url: str = "") -> Dict[str, Any]:
        ...
        complexity = self.calculate_complexity_metrics(lemmas_all, full_text)
        intent = self.get_intent(lemmas_all, all_words)
    ...
    def process_analysis(
        ...
        async def _fetch_all_competitors(urls: List[str]) -> List[Dict]:
        ...
        comp_results = asyncio.run(_fetch_all_competitors(comp_urls))
        ...
        for lemma, _ in top_20_words:
            ...
            top_20_stats.append(
                {"lemma": lemma, "median_density": median_density, "target_density": target_density}
            )
        ...
        report = {
            ...
            "text_metrics": {
                ...
                "competitors_median": {
                    ...
                    "words_total": int(
                        np.median([c["metrics"]["words_total"] for c in comp_results])
                    )
                    if comp_results
                    else 0,
                    "words_unique": int(
                        np.median([c["metrics"]["words_unique"] for c in comp_results])
                    )
                    if comp_results
                    else 0,
                    ...
                    "wateriness": round(
                        np.median([c["metrics"]["wateriness"] for c in comp_results]), 2
                    )
                    if comp_results
                    else 0,

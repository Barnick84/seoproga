...
def restore_minus_words(user_id, domain, keywords):
    ...
    try:
        ...
        if updated == 0 and len(keywords) > 0:
            return {
                "success": False,
                "error": "Ни один запрос не найден в базе для этих параметров",
                "updated": 0,
            }
        return {"success": True, "updated": updated}
    except Exception as e:
        sys.stderr.write(f"ERROR in restore_minus_words: {str(e)}\n")
...
if __name__ == "__main__":
            ...
            if input_data:
                ...
                print(
                    json.dumps(
                        {
                            "success": False,
                            "error": "No data provided via args or stdin",
                        }
                    )
                )
                sys.exit(1)
    ...
    result = restore_minus_words(user_id, domain, keywords)

...
def prepare_brief(domain, cluster_id, user_id):
    try:
        ...
        brief_response = client.chat.completions.create(
            model=model, messages=[{"role": "user", "content": brief_prompt}]
        )
        ...
        rewrite_response = client.chat.completions.create(
            model=model, messages=[{"role": "user", "content": rewrite_prompt}]
        )
    ...
    if len(sys.argv) < 4:
        print(
            json.dumps(
                {
                    "success": False,
                    "error": "Usage: prepare_seo_brief.py <domain> <cluster_id> <user_id>",
                }
            )
        )
    else:

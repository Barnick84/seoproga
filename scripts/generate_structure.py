...
def generate_structure_task(
    domain: str, user_id: int, cluster_id: int, keywords: list[str]
) -> dict:
    try:
    ...
    if len(sys.argv) < 5:
        print(
            json.dumps(
                {
                    "success": False,
                    "error": "Usage: generate_structure.py <domain> <user_id> <cluster_id> <keywords_json>",
                }
            )
        )
        return

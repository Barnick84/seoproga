...
def find_relevant_page(domain: str, main_keyword: str) -> str | None:
    ...
    try:
        ...
        base_domain = extract_domain(
            "https://" + domain if not domain.startswith("http") else domain
        )
        for url in urls:
...
def run_seo_analysis_task(domain: str, cluster_id: int, user_id: int) -> dict:
    conn = None
    try:
        ...
        if not keywords_rows:
            return {"success": False, "error": "Нет ключевых слов в кластере"}
        ...
        return {"success": True, "message": f"SEO анализ завершён для {main_keyword}"}
    ...
    if len(sys.argv) < 3:
        print(
            json.dumps(
                {
                    "success": False,
                    "error": "Usage: run_seo_analysis.py <domain> <cluster_id> [user_id]",
                }
            )
        )
        return

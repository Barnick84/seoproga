...
def run_daily_update():
    ...
    try:
        ...
        cur.execute(
            "SELECT id, username, yandex_token FROM users WHERE is_blocked = 0 AND yandex_token IS NOT NULL"
        )
        users = cur.fetchall()
        ...
        conn.close()
        print(
            f"[{datetime.now()}] ✅ Update finished. Sites: {total_sites}, Queries: {total_queries}"
        )
